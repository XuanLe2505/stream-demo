...
import ShareScreenIcon from "../common/ShareScreen";
...
export default function RoomDetail() {
  ...
  const toggleCamera = async () => {
    if (stream) {
      ...
      if (isCamera) {
        ...
        Object.values(peers as peerStateType).forEach((peer) =>
          replaceTrack(peer.peer, newVideoTrack)
        );
      }
    }
  ...
  const handleShareScreen = async () => {
    const isShareScreenTemp = !isShareScreen;
    let streamTemp: MediaStream;
    stream?.getTracks().forEach((track) => track.stop());
    if (isShareScreenTemp) {
      streamTemp = await navigator.mediaDevices.getDisplayMedia({});
    } else {
      streamTemp = await navigator.mediaDevices.getUserMedia({
        audio: true,
        video: true,
      });
    }
    setStream!(streamTemp);
    Object.values(myPeer?.connections!).forEach((connection: any) => {
      const videoTrack = streamTemp
        ?.getTracks()
    ...
    setIsShareScreen(isShareScreenTemp);
  };
    ...
    <main className="relative min-h-screen w-full p-5">
      ...
      <div className="mt-4 grid grid-cols-2 gap-4">
        ...
        {Object.values(peers).length > 0 &&
          Object.values(peers as peerStateType).map((peer) => (
            <VideoPlayer stream={peer.stream} key={peer.peer.peer} />
          ))}
      </div>
      <div className="fixed bottom-0 left-0 right-0 flex justify-center gap-4 py-5">
        ...
        <div
          className="relative cursor-pointer w-12 h-12 border border-black rounded-full"
