...
export type peerStateType = Record<
  string,
  { peer: MediaConnection; userName?: string; stream?: MediaStream }
>;
...
type addAllPeersType = {
  ...
  payload: {
    peers: Record<string, { peer: MediaConnection; userName: string }>;
  };
};
  ...
  switch (action.type) {
    ...
    case REMOVE_PEER_STREAM:
      if (state) {
        ...
        delete stateTemp[action.payload.peerId];
        return { ...stateTemp };
      }
    default:
      return { ...state };
